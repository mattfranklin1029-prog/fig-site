/* public/js/helpers.js
   Franklin Innovation Group — site helpers (safe on all pages)
   - Image fallback chain (avif → webp → jpg → png → jpeg → jfif)
   - Footer year(s)
   - Animated counters (respects prefers-reduced-motion)
   - Before/After slider
   - Lightbox for .lb-thumb images
   - Project modals
   - Lazy loader for Bertline dashboard (/js/bertline.js)
*/
(() => {
  // ---------- tiny DOM helpers ----------
  ...
  // ---------- 1) Image fallback chain ----------
  const EXT_CHAIN = ['avif','webp','jpg','png','jpeg','jfif'];
  function applyFallback(img) {
    try {
      const srcAttr = img.getAttribute('src') || '';
      const base = img.dataset.base || srcAttr.replace(/\.(avif|webp|jfif|jpeg|jpg|png)(\?.*)?$/i, '');
      let i = 0;
      const currentExtMatch = srcAttr.match(/\.(\w+)(\?.*)?$/);
      if (currentExtMatch) {
      ...
      function tryNext() {
        ...
        const next = `${base}.${EXT_CHAIN[i++]}`;
        const probe = new Image();
        probe.onload  = () => { img.src = next; img.removeAttribute('data-fallback-active'); };
        probe.onerror = tryNext;
        img.setAttribute('data-fallback-active','1');
        probe.src = next + (next.includes('?') ? '&' : '?') + 'v=' + EXT_CHAIN[i-1];
      }
  ...
  function initBeforeAfter() {
    ...
    if (!topLayer || !handle || !line) return;
  ...
  function initLightbox() {
    ...
    const thumbs = $$('.lb-thumb');
    if (!lbImg || !prevBtn || !nextBtn || !closeBtn || !thumbs.length) return;

    thumbs.forEach(applyFallback);

    let idx = -1, touchStartX = 0;
    const srcFor = i => (thumbs[i].dataset.src || thumbs[i].currentSrc || thumbs[i].src || '');
    function updateLB() {
      if (idx < 0) return;
      lbImg.src = srcFor(idx);
      lbImg.alt = thumbs[idx].alt || '';
      [ (idx+1)%thumbs.length, (idx-1+thumbs.length)%thumbs.length ].forEach(j => { const p = new Image(); p.src = srcFor(j); });
    }
    function openLB(i) {
      idx = i; updateLB();
  ...
  // ---------- 6) Project modals ----------
  function initModals() {
    const triggers = $$('.project-trigger');
  ...
  // ---------- 7) Lazy-load Bertline dashboard ----------
  // Loads /js/bertline.js ONLY on pages that need it (detected by IDs or data-flag).
  function initBertlineLazyLoader() {
    const needsBert =
      ['incidents-chart','aging-chart','deploys-chart','cfr-pie'].some(id => document.getElementById(id)) ||
      document.querySelector('[data-bertline]');

    if (!needsBert) return;

    // Use dynamic import; bertline.js will inject AG Charts if missing.
    import('/js/bertline.js')
      .then(m => m.initBertline?.())
      .catch(err => console.warn('Bertline module failed to load:', err));
  }

  // ---------- DOM ready orchestrator ----------
